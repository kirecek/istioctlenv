...
check_active_version() {
  local v="${1}";
  [ -n "$(${TFENV_ROOT}/bin/istioctl version --remote=false --short)" ];
}
export -f check_active_version;

check_installed_version() {
  local v="${1}";
  local bin="${TFENV_ROOT}/versions/${v}/bin/istioctl";
  [ -n "$(${bin} version --remote=false --short)" ];
};
...
cleanup() {
  ...
  rm -rf ./versions;
  log 'debug' "Deleting ${pwd}/.istioctl-version";
  rm -rf ./.istioctl-version;
};
export -f cleanup;
